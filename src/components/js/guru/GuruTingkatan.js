...
import { getKelas, Url, useTitle } from '../utils';

export const tingkatanUrl = 'http://localhost/nsejarah-react/api/tingkatan.php';


export async function getTingkatan( idTing = null )
{
    const tambahan = idTing !== null ? `?id_ting=${idTing}` : '';
    const request = await fetch( tingkatanUrl + tambahan );
...
export async function getTingkatanByGuru( idGuru )
{
    const request = await fetch( tingkatanUrl + '?id_guru=' + idGuru );
